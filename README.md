...
- **头像/图标磁盘缓存**: 头像默认 12 小时刷新周期(可配置), 刷新失败自动沿用旧缓存；图标保留最后一次成功获取的值；重启不丢, `/motdconfig refresh` 立即刷新(v2.1.0+)
- **会话控制**: 可设置对哪些群聊/私聊生效
- **管理员权限**: 插件配置仅管理员可修改
- **完善报错**: 详细的错误提示和帮助信息
...
| `/motdconfig refresh` | 清空头像/图标缓存, 下次查询重新下载(立即刷新) | `/motdconfig refresh` |
...
1. 确认服务器地址和端口是否正确
2. 保持 `use_api: true`(默认开启)开启 API 竞速查询(API 与直连并发, 谁先返回用谁)
...
- v2.1.0 起头像由插件预取并内嵌到渲染图(mc-heads → crafatar → minotar 三源自动回退 + 磁盘缓存, 默认 12 小时刷新周期), 加载失败率大幅降低；刷新失败自动沿用旧头像, 从未成功获取过的玩家才回退为「首字母 + 哈希色」占位块, 不影响卡片其余部分
- 头像显示为旧皮肤时属正常缓存现象, 可执行 `/motdconfig refresh` 立即清空缓存, 或调低 `avatar_cache_ttl`
...
[MOTD] 使用 API 查询: True
[MOTD] 头像预取: 开启, 头像缓存 12 小时, 图标缓存 24 小时
[MOTD] 插件已加载 vX.X.X
```
